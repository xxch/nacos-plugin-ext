...
public class HistoryConfigInfoMapperByPostgresql extends HistoryConfigInfoMapperByMySql {
    ...
    public MapperResult removeConfigHistory(MapperContext context) {
        String sql = "WITH temp_table AS (SELECT nid FROM his_config_info WHERE gmt_modified < ? ORDER BY nid LIMIT ?) "
                + "DELETE FROM his_config_info WHERE nid IN (SELECT nid FROM temp_table)";
